...
The `http-custom-mechanism` quickstart demonstrates how to implement a custom HTTP mechanism and then register it with Elyton. This makes it possible to override the configuration within the deployment to make use of this mechanism without requiring modifications to the deployment. This example make use of custom HTTP headers to receive a clear text username and password and use them for authentication.

IMPORTANT: Generally you should avoid passing clear text passwords. It is only done here to simplify the example.
...
| Root folder
a| The project in the root folder contains a deployable web application consisting of a secured servlet that is used to test the configuration. It generates the `{artifactId}.war` archive used to test the secured application.

| `custom-module/`
a| This project contains the source for custom HTTP authentication module that overrides the deployment configuration. It contains of the following Java classes and resources.

* `CustomHeaderHttpAuthenticationMechanism`: This class implements `HttpServerAuthenticationMechanism`.
* `CustomMechanismFactory`: This class implements `HttpServerAuthenticationMechanismFactory`.
* `org.wildfly.security.examples.CustomMechanismFactory`: This resource file contains a single line that names the custom mechanism factory class, which in this case is `org.jboss.as.quickstart.http_custom_mechanism.CustomMechanismFactory`.
|===
...
By default, the deployed application uses the `other` security domain. We need to map this security domain to an Elytron HTTP authentication factory. You can configure this mapping by running the following CLI command.

For your convenience, this quickstart includes the command to configure the security domain in the `configure-security-domain.cli` script located in the root directory of this quickstart.
...
[source,options="nowrap"]
----
WFLYUT0021: Registered web context: '/http-custom-mechanism' for server 'default-server'
WFLYSRV0010: Deployed "http-custom-mechanism.war" (runtime-name : "http-custom-mechanism.war")
----

[[test_the_secured_servlet]]
== Test the Secured Servlet

Before you continue, you must test the secured servlet deployment to make sure it is working. Since this application uses a standard mechanism, it could be tested using a browser. However, after you implement the custom HTTP mechanism, the browser will not understand, so it is better to test the call using a client that will allow you to manipulate the headers yourself.

Issue the following command to test the deployment.

[source,options="nowrap"]
----
curl -v http://localhost:8080/http-custom-mechanism/secured -u testuser:Password1!
----
...
You configure the server by running JBoss CLI commands. For your convenience, this quickstart batches the commands into a `configure-elytron.cli` script provided in the root directory of this quickstart.
...
[source,options="nowrap"]
----
curl -v http://localhost:8080/http-custom-mechanism/secured -H "X-USERNAME:testuser" -H "X-PASSWORD:password"
